...
class EstoqueApp:
    ...
    def create_widgets(self):
        ...
        self.tree = ttk.Treeview(
            self.root,
            columns=("id", "nome", "codigo_produto", "descricao", "localizacao"),
            show="headings",
        )
        ...
        try:
            ...
            for p in pecas:
                self.tree.insert(
                    "",
                    "end",
                    values=(p["id"], p["nome"], p["codigo_produto"], p["descricao"], p["localizacao"]),
                )
        except Exception as e:
    ...
    def buscar_pecas(self):
        ...
        try:
            headers = {"Authorization": f"Bearer {self.token}"} if self.token else {}
            resp = requests.get(f"{API_URL}/", params={"nome": nome}, headers=headers)
            resp.raise_for_status()
            pecas = resp.json()
            self.tree.delete(*self.tree.get_children())
            for p in pecas:
    ...
    def usuario_form(self, title):
        ...
        for i, (campo, label) in enumerate(campos):
            ...
            if campo == "password" or campo == "confirm_password":
                ...
                entries[campo] = ttk.Combobox(
                    main_frame, width=22, values=["estoquista", "admin", "vendedor"], state="readonly"
                )
        ...
        def salvar_usuario():
            ...
            # Dados para envio
            dados = {"username": username, "password": password, "role": role}
                ...
                if resp.status_code == 201:
                    ...
                    if "Usuário já existe" in error_data.get("detail", ""):
                        ...
                        messagebox.showerror(
                            "Erro", f"Erro na criação: {error_data.get('detail', 'Erro desconhecido')}"
                        )
        ...
        tk.Button(
            btn_frame, text="Cancelar", command=cancelar, bg="#f44336", fg="white", relief=tk.RAISED, width=10
        ).pack(side=tk.LEFT, padx=5)
